...
up: setup ## Start shared infra only (collector + 3 stores) — point your own apps at :4318
	AGENTOTEL_STACK_UUID="$$(./bin/obs stack-id)" docker compose up -d

demo: setup ## Start infra + the bundled sample app (profile: demo)
	AGENTOTEL_STACK_UUID="$$(./bin/obs stack-id)" docker compose --profile demo up -d --build
...
.PHONY: setup install uninstall up down clean doctor storage disk cardinality canary reset migrate logs load smoke security-test cardinality-test identity-test storage-test ci-local dashboard grafana grafana-down e2e mcp-build mcp-test mcp-race ps help
