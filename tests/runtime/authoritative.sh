...
git init -q "$t/repo"; cd "$t/repo"; git config user.email test@example.invalid; git config user.name test; AGENTOTEL_DEV_MODE=1 "$root/bin/obs" init >/dev/null; AGENTOTEL_DEV_MODE=1 "$root/bin/obs" project rekey >/dev/null; AGENTOTEL_DEV_MODE=1 "$root/bin/obs" source-state | grep -q checkout_id
