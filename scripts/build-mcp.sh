...
docker run --rm -v "$ROOT/mcp:/src:ro" -v "$tmp:/out" -w /src golang:1.26.5-bookworm@sha256:53eeac89074db483fdf0ab3be1df32bf6e47562263d2d0d6baa7f26acb4957dd sh -c "CGO_ENABLED=0 GOOS=$goos GOARCH=$goarch go build -trimpath -ldflags '-X main.buildVersion=$version' -o /out/agentotel-mcp ." >/dev/null
