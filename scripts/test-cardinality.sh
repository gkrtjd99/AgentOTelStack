...
grep -A12 'transform/resource-allowlist:' otel-collector/config.yaml | grep -Fq 'keep_keys(resource.attributes, ["project", "service.name", "deployment.environment"])' || { echo 'cardinality-test: resource sanitizer must use the bounded allowlist' >&2; exit 1; }
metric_block="$(awk '/^  transform\/metric-sanitize:/{seen=1} seen && /^  [^[:space:]][^:]*:/{if ($0 !~ /transform\/metric-sanitize:/) exit} seen{print}' otel-collector/config.yaml)"
