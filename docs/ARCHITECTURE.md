...
| `correlate.sh <traceID>` | Traces + Logs + Metrics | Jaeger API + LogQL + PromQL | 아래 §4.2 |
| `app.sh <subcmd> ...` | 서비스별 조회 helper | LogQL + PromQL + Jaeger API | `service.name`/`service_name` 차이를 숨김 |
| `overview.sh [--compact\|--json] [service]` | 터미널 overview | LogQL + PromQL + Jaeger API | 사람이 빠르게 보는 요약 |
...
3. Trace 응답의 service name을 뽑아 **VictoriaMetrics**에서 같은 서비스의 최근 metric snapshot을 출력
   (`orders_processed_total`, error rate, p95 latency 등 — 앱이 해당 metric을 내보낼 때).

→ "뭔가 느리다/깨졌다"에서 → "정확히 이 코드 경로 + 이 로그 맥락 + 같은 서비스의 최근 수치"로 점프.
...
3. 상관관계    obs/logs.sh ...severity_text:error  → 에러 로그 1건의 trace_id 추출
              obs/correlate.sh <trace_id> # 스팬 + 로그 + 같은 서비스 metric snapshot을 한 번에
...
> 트레이스+로그가 *어디서, 왜* 를 알려주고, metric snapshot이 같은 서비스의 최근 상태를 보강한다.
...
| Grafana | 3001 | 선택형 browser dashboard (`make grafana`) |
