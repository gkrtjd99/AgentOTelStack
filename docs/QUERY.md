...
Every current query envelope carries a namespaced `kind` (`gateway.services.v1`,
`gateway.context.v1`, `gateway.errors.v1`, or `gateway.correlate.v1`), typed
`scope`, and RFC3339 `freshness`. `freshness` is the Gateway query cutoff, not
the time at which telemetry was ingested or proof that every backend signal is
complete. Compare `scope` and `freshness` when comparing before/after workload
runs; preserve `partial`, `truncated`, and backend statuses as separate
limitations. Dashboard `fetched_at` is generated by the Dashboard and must not
be treated as raw Gateway `freshness`.
